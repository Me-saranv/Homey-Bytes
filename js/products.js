...
function renderProductsList() {
    ...
    if (products.length === 0) {
        list.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: #999; padding: 2rem;"><p>📦 No products yet</p><p style="font-size: 0.9rem; margin-top: 0.5rem;">Click "➕ Add Product" to create your first snack!</p></div>';
        return;
    }
    ...
    list.innerHTML = products.map(p => `
        <div class="product-card-admin" style="border: 2px solid var(--light-purple); transition: all 0.3s;">
            <div class="product-card-admin-img">
                ${p.image ? `<img src="${p.image}" alt="${p.name}" style="cursor: pointer;" onclick="previewProductImage('${p.image}')">` : p.emoji}
            </div>
